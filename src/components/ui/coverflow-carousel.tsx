...
export interface CoverflowCarouselProps {
  ...
  /** Fires when the centred slide settles on a new index. */
  onSelect?: (index: number) => void;
  /** Fires when the user taps the already-centred card (no drag). */
...
export function CoverflowCarousel({
  ...
  /** Total pointer travel this gesture — distinguishes a tap from a drag. */
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    movedRef.current += Math.abs(event.clientX - drag.x);
    ...
    >
      <div className="relative">
        <div
          ...
          className="cursor-grab overflow-hidden py-10 outline-none ring-slate-300 focus-visible:ring-2 active:cursor-grabbing"
          style={{
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  "absolute left-1/2 top-0 aspect-square overflow-hidden rounded-2xl bg-slate-100 shadow-xl ring-1 ring-slate-900/5 will-change-transform",
                  cardClassName,
                )}
              ...
              >
                ...
                />
                {slide.title && (
                  <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-slate-900/70 to-transparent p-3">
                    <p className="text-[13px] font-semibold tracking-tight text-white">
                      {slide.title}
                    </p>
                    {slide.subtitle && (
                      <p className="text-[11px] text-white/70">{slide.subtitle}</p>
                    )}
                  </div>
                )}
              </div>
        ...
        {showNavigation && (
          <>
            <button
              ...
              onClick={() => nudge(-1)}
              className="absolute left-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-white/80 p-2 text-slate-900 shadow-md backdrop-blur transition hover:bg-white"
            >
            ...
            <button
              ...
              onClick={() => nudge(1)}
              className="absolute right-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-white/80 p-2 text-slate-900 shadow-md backdrop-blur transition hover:bg-white"
            >
      ...
      {showCaption && active?.title && (
        ...
        >
          ...
          {active.subtitle && (
            <p className="mt-1 text-[13px] text-slate-500">{active.subtitle}</p>
          )}
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          {slides.map((_, index) => (
            <button
              ...
              onClick={() => goTo(index)}
              className={cn(
                "size-2 rounded-full bg-slate-900 transition-opacity",
